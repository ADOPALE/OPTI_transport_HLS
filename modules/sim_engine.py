...
def est_compatible_sj_et_job(sj_jobs, nouveau_job, matrice_duree, df_vehicules, df_sites, taux_max_cible, df_contenants=None):
    ...
    # --- 0. VÉRIFICATION COMPATIBILITÉ VÉHICULE/CONTENANT ---------------
    # Le type de véhicule du SuperJob est celui du pivot.
    # Pour les jobs INCOMPLET uniquement, on lève la contrainte d'identité
    # de véhicule : si le contenant du candidat est accepté par le véhicule
    # du pivot, le groupage est évalué même si le candidat aurait normalement
    # un véhicule différent.
    # Les jobs COMPLET ne peuvent être regroupés qu'avec le même type de véhicule.
    v_type_pivot = sj_jobs[0].vehicule_type
    v_type = v_type_pivot

    if nouveau_job.type_job == 'COMPLET':
        # Job complet : véhicule identique obligatoire
        if nouveau_job.vehicule_type != v_type_pivot:
            return False
    else:
        # Job incomplet : on vérifie uniquement que son contenant
        # est compatible avec le véhicule du pivot
        v_info_check = df_vehicules[df_vehicules.iloc[:, 0] == v_type_pivot]
        if not v_info_check.empty:
            cont = str(nouveau_job.contenant).strip()
            if cont in v_info_check.columns:
                accepte = str(v_info_check[cont].values[0]).strip().upper()
                if accepte != "OUI":
                    return False
            else:
                # Contenant inconnu dans df_vehicules → refus par sécurité
                return False
